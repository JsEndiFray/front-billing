...
export class ValidatorService {
  ...
  private transformInvoiceFields(controls: { [key: string]: AbstractControl }): void {
    // Los números ya los maneja Angular Forms automáticamente
    // Solo limpiamos strings si es necesario
    if (controls['collection_notes']?.value) {
      controls['collection_notes'].setValue(controls['collection_notes'].value.toString().trim(), { emitEvent: false });
    }
  }
  ...
  /**
   * Valida existencia real de referencia catastral en el Catastro español
   * Consulta la API oficial del Ministerio de Hacienda
   */
  /*async validateCadastralReferenceExists(refCat: string): Promise<{ isValid: boolean; message?: string }> {
    // 1. Primero validar el formato
    ...
    // 2. Consultar existencia real en API del Catastro
    try {
      // API oficial del Catastro español para consultar referencia catastral
      const apiUrl = `https://ovc.catastro.meh.es/ovcservweb/OVCSWLocalizacionRC/OVCCallejero.asmx/Consulta_DNPRC?Provincia=&Municipio=&RC=${refCat}`;
      ...
      if (!response.ok) {
        // Si hay error HTTP, validamos solo formato
        return {
          isValid: true,
      ...
      const errorElements = xmlDoc.getElementsByTagName('err');
      if (errorElements.length > 0) {
        const errorDesc = errorElements[0].getAttribute('des') || 'Referencia no encontrada';

        // Errores más comunes del Catastro
      ...
      const coordenadas = xmlDoc.getElementsByTagName('coord');
      const direccion = xmlDoc.getElementsByTagName('ldt');

      if (coordenadas.length > 0 && direccion.length > 0) {
        // La referencia existe y tiene datos válidos
        return { isValid: true };
      } else {
        // No hay datos válidos, probablemente no existe
        return {
          isValid: false,
          message: 'La referencia catastral no existe o no tiene datos válidos en el Catastro'
        };
      }

    } catch (error) {
      // Si hay cualquier error de red o parsing, validamos solo formato
  ...
  validatePercentage(percentage: number | string | null | undefined, fieldName: string = 'porcentaje'): { isValid: boolean; message?: string } {
